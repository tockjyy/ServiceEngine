...
word SubWord(word sw)
{
	...
	{
		int row = sw >> (i * 8 + 4) & 0x0F;
		int col = sw >> (i * 8) & 0x0F;
		byte val = S_Box[row][col];
		for (int j = 0; j < 8; ++j)
			memcpy(&temp + i, &val, 1);// temp[i + j] = val[j];
	}
	return temp;
}
...
//设置秘钥
bool AES_128::Set_Password(const string & pwd)
{
	if (pwd.size() != 16)return false;
	byte key[16];
	memset(key, 0x00, 16);
	memcpy(key, pwd.c_str(), pwd.size());
	KeyExpansion(key, password);
	return true;
}

//解密
string AES_128::DeCrypt(const string& context)
{
	string result;
	auto count = context.size();
	auto pstr = context.c_str();
	int loop(0);
	while (loop < count)
	{
		size_t s = count - loop;
		...
		memcpy(plain, pstr+loop, s);
		loop += 16;
		decrypt(plain, password);
		result.append((char *)plain, 16);
	}
	return move(result);
}
//加密
string AES_128::EnCrypt(const string& context)
{
	string result;
	auto count = context.size();
	auto pstr = context.c_str();
